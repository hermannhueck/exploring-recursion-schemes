...
object FirstExample extends util.App {

  // So, if you’re reading this, you probably already have an AST or other recursive ADT somewhere
  // that has gotten a bit out of hand, and you’ve heard that Matryoshka (or something about recursion schemes) may help you.

  // So, let’s start with a simple version of what you may already have.
  // Here’s an AST representing a few arithmethic operations.
  // Like most ASTs, it’s recursive – the case classes refer directly to the abstract class they’re extending.
  ...
  // We gain a lot of flexibility by separating that recursion from the AST.
  // We do this with the Stephen Compall mantra of “add a type parameter!”
  ...
  // So, everywhere that referred to Arithmetic before now just refers to A,
  // and we can fill that with whatever type we want. And, with a type parameter,
  // come the type class instances (this should be derivable, but doesn’t yet exist in shapeless-contrib)

  import scalaz._, Scalaz._

  implicit val traverse: Traverse[ArithmeticF] = new Traverse[ArithmeticF] {
    def traverseImpl[G[_], A, B](fa: ArithmeticF[A])(f: A => G[B])(implicit G: Applicative[G]) =
      fa match {
  ...
  // Since you already have your recursive AST, you also have code that uses it,
  // and the last thing you want to do when trying a new tool is have to rewrite the code you already have.
  // So, we’ll keep your AST and your code – all you have to do is define the relationship
  // between the original AST and the new one, which we’ll do like this:
  ...
  // All that this does is define the mapping between the directly-recursive structure and the functor,
  // so it’s just a matter of seeing where the F suffix is.
  ...
  val pretty: String = Recursive[Arithmetic].cata(expr)(prettyPrint)
  pretty pipe println

  // NB: This is just a simple example. There are much better ways to print ASTs
  // (which also take advantage of Matryoshka), but they are too complicated for this example.
  ...
  Recursive[Arithmetic].cata(expr)(eval) pipe println

  // So, that’s two simple examples where we don’t have to think about recursion at all
  // – only one level of the operation at a time. But, we can now use this to gain some efficiency:

  val zippedAlgebras = Zip[Algebra[ArithmeticF, ?]].zip(eval, prettyPrint)
  Recursive[Arithmetic].cata(expr)(zippedAlgebras) pipe println

  // Now we’ve generated both values, but we only traversed the tree once. This is a pattern we see a lot in recursion schemes – ways to compose operations at the algebra level, such that we can do multiple operations in a single pass. And there are a bunch of “algebra transformations”, which massage these algebras into the shapes needed for different kinds of composition, so you don’t need to think about that at the point at which the algebra is defined.
  ...
  def buildOp(currentPrecedence: Int, a: (Int, String), op: String, b: (Int, String)): String = {
  ...
  Recursive[Arithmetic].zygo(expr)(precedence, prettyPrintʹ) pipe println
