...
object Algebras extends util.App {

  // The algebras (and coalgebras, etc.) are more properly known as “F-algebras”.
  // To connect these back to the algebra we’re all generally familiar with from school,
  // let’s look at a simple ADT –
  ...
  val expr: Mu[Expr] = Add(
    Mul(Add(Num[Mu[Expr]](2).embed, Num[Mu[Expr]](3).embed).embed, Num[Mu[Expr]](4).embed).embed,
    Add(Mul(Num[Mu[Expr]](5).embed, Num[Mu[Expr]](6).embed).embed, Num[Mu[Expr]](7).embed).embed
  ).embed
  ...
  // Hey, look at that – this evaluation rule is an “algebra”.
  // And it’s just a simplified version of the particular algebra we grew up with.

  // The second aspect involves applying these rules from most deeply nested expressions out.

  expr.cata(eval) pipe println

  // There it is. You’ve got an algebra and applied it.

  // Whence from here?

  // Well, you’re already familiar with algebraic data types in general
  // (which get their name via a different path from algebra).
}
